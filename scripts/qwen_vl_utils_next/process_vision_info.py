...
def _read_video_vr_rs(
    ...
    """read video using video_reader-rs.PyVideoReader
    ...
    if isinstance(ele["video"], bytes):
        raise NotImplementedError("Not support bytes video for video_reader-rs.PyVideoReader")
    else:
        video_path = ele["video"]
        vr = PyVideoReader(video_path, log_level="panic")
    if "video_start" in ele or "video_end" in ele:
        raise NotImplementedError("not support start_pts and end_pts in video_reader-rs.PyVideoReader for now.")
    ...
    # resize & get_batch frames
    filter = f"format=yuv420p,scale=w={resized_width}:h={resized_height}:flags=fast_bilinear"
    vr = PyVideoReader(video_path, filter=filter, log_level="panic", oob_mode="black")
...
def get_video_reader_backend() -> str:
    ...
    elif is_vr_rs_available():
        video_reader_backend = "vr_rs"
...
def fetch_video(
    ...
    VIDEO_FRAME_MIN_PIXELS = VIDEO_MIN_TOKEN_NUM * image_factor * image_factor
    VIDEO_FRAME_MAX_PIXELS = VIDEO_MAX_TOKEN_NUM * image_factor * image_factor
    ele["image_patch_size"] = image_patch_size
...
def process_vision_info(
    ...
    Optional[List[Image.Image]], Optional[List[Union[torch.Tensor, List[Image.Image]]]], Optional[Dict[str, Any]]
]:
...
def reformat_message_vid2multiframe(
    conversations: Union[List[Dict[str, Any]], List[List[Dict[str, Any]]]],
    image_patch_size: int = 14,
    jpeg_quality: int = 90,
) -> Union[List[Dict[str, Any]], List[List[Dict[str, Any]]]]:
    """
    Traverse the entire conversations, find all video items in content,
    and replace them with multi-frame image inputs.

    Each video will be replaced with: [timestamp_text, image, timestamp_text, image, ...]

    Args:
        conversations: List of conversations, can be a single conversation or list of conversations
        image_patch_size: Image patch size for resize calculation
        jpeg_quality: JPEG encoding quality (1-100)

    Returns:
        Converted conversations with video replaced by multi-frame images
    """
    import cv2

    # Determine input format
    is_single_conversation = isinstance(conversations[0], dict) and "content" in conversations[0]
    ...
    for conversation in conversations:
        result_conversation = []
        for message in conversation:
            # Deep copy message to avoid modifying original data
            new_message = copy.deepcopy(message)

            # Only process when content is a list
            if not isinstance(new_message.get("content"), list):
                result_conversation.append(new_message)
                continue
            ...
            for item in new_message["content"]:
                ...
                if "video" in item or item.get("type") == "video":
                    # Extract video and convert to multi-frame
                    try:
                        video_data, video_metadata, _ = _read_video_vr_rs(
                            ele=item,
                            image_patch_size=image_patch_size,
                            return_torch_tensor=False,
                        )
                        logger.info(
                            f"vide_data shape: {video_data.shape}, fps: {video_metadata.get('fps', None)}, frames_indices: {video_metadata.get('frames_indices', None)}"
                        )

                        # Convert each frame to base64 image
                        fps = video_metadata.get("fps", 25.0)
                        frames_indices = video_metadata.get("frames_indices", [])
                        ...
                        for idx, frame_idx in enumerate(frames_indices):
                            ...
                            # Add timestamp text
                            new_content.append({"type": "text", "text": timestamp_text})

                            # numpy RGB -> BGR -> encode JPEG
                            _, jpeg_bytes = cv2.imencode(
                                ".jpg",
                                cv2.cvtColor(video_data[idx], cv2.COLOR_RGB2BGR),
                                [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality],
                            )
                            jpeg_b64 = base64.b64encode(jpeg_bytes.tobytes()).decode("utf-8")
                        ...
                        logger.warning(f"Failed to convert video to multi-frame: {e}")
                        # Keep original video item on failure
                        new_content.append(item)
                else:
                    # Non-video type, keep as is
                    new_content.append(item)

            new_message["content"] = new_content
            result_conversation.append(new_message)

        result_conversations.append(result_conversation)

    # Return same format as input
    if is_single_conversation:
        return result_conversations[0]
